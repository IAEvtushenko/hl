...
module.exports = function(XRegExp) {
    ...
    function cacheInvertedBmp(pk) {
        var prop = 'b!';
        return (
            unicode[pk][prop] ||
            (unicode[pk][prop] = invertBmp(unicode[pk].bmp))
        );
    }

    // Combines and optionally negates BMP and astral data
    function buildAstral(pk, isNegated) {
        var item = unicode[pk];
        var combined = '';
    ...
    function cacheAstral(pk, isNegated) {
        var prop = isNegated ? 'a!' : 'a=';
        return (
            unicode[pk][prop] ||
            (unicode[pk][prop] = buildAstral(pk, isNegated))
        );
    }
    ...
    XRegExp.addToken(
        ...
        function(match, scope, flags) {
            ...
            var pk = normalize(match[4] || match[3]);
            // Token data object
            var item = unicode[pk];
            ...
            if (!unicode.hasOwnProperty(pk)) {
                throw new SyntaxError(ERR_UNKNOWN_NAME + match[0]);
            }

            // Switch to the negated form of the referenced Unicode token
            if (item.inverseOf) {
                pk = normalize(item.inverseOf);
                if (!unicode.hasOwnProperty(pk)) {
            ...
            if (isAstralMode) {
                ...
                return cacheAstral(pk, isNegated);
            }

            return scope === 'class' ?
                (isNegated ? cacheInvertedBmp(pk) : item.bmp) :
                (isNegated ? '[^' : '[') + item.bmp + ']';
        },
        {
            scope: 'all',
